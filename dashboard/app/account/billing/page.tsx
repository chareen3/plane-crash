...
import { Orbit, CreditCard, Landmark, Loader2, AlertCircle, LogOut, ArrowLeft, RefreshCw, Calendar, CheckCircle, Download } from 'lucide-react'
import { flagPostSubscribeInstall } from '../../components/pwa/pwa-utils'
import { usePwa } from '../../components/pwa/PwaProvider'

function BillingInner() {
  const router = useRouter()
  const searchParams = useSearchParams()
  const { openInstallSheet, isInstalled } = usePwa()
  ...
  // Post-checkout success → flag install prompt
  useEffect(() => {
    if (searchParams?.get('success') === 'true') {
      setJustSubscribed(true)
      flagPostSubscribeInstall()
      if (!isInstalled) {
        const t = window.setTimeout(() => openInstallSheet('subscribe'), 700)
        return () => window.clearTimeout(t)
      }
    }
  }, [searchParams, isInstalled, openInstallSheet])
  ...
  return (
    <div className="min-h-screen bg-[#080c18] text-[#f0f4ff] overflow-x-hidden flex flex-col justify-between relative">
      ...
      <main className="relative z-10 max-w-4xl mx-auto px-6 py-12 flex-grow w-full space-y-8">
        ...
        {justSubscribed && (
          <div className="p-4 rounded-2xl border border-emerald-500/25 bg-emerald-500/10 text-emerald-300 text-sm flex flex-col sm:flex-row sm:items-center gap-3">
            <div className="flex items-start gap-3 flex-1">
              <CheckCircle size={20} className="shrink-0 mt-0.5" />
              <div>
                <div className="font-bold text-emerald-200">Subscription activated</div>
                <div className="text-xs text-emerald-300/80 mt-1">
                  Install CrashTracker on your phone for one-tap access to live AI signals.
                </div>
              </div>
            </div>
            <div className="flex gap-2 shrink-0">
              {!isInstalled && (
                <button
                  type="button"
                  onClick={() => openInstallSheet('subscribe')}
                  className="inline-flex items-center justify-center gap-2 px-4 py-2.5 rounded-xl bg-gradient-to-r from-cyan-400 to-blue-500 text-slate-950 text-xs font-black"
                >
                  <Download size={14} /> Install app
                </button>
              )}
              <Link
                href="/app?welcome=subscribed"
                className="inline-flex items-center justify-center gap-2 px-4 py-2.5 rounded-xl border border-emerald-400/30 text-emerald-200 text-xs font-bold"
              >
                Open dashboard
              </Link>
            </div>
...
export default function BillingPage() {
  return (
    <Suspense
      fallback={
        <div className="min-h-screen bg-[#080c18] flex items-center justify-center text-[#5a6a8a] text-xs gap-2">
          <Loader2 className="animate-spin text-cyan-400" size={16} /> Loading billing…
        </div>
      }
    >
      <BillingInner />
